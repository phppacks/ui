<template>
  <div :class="`${prefixCls}-btn-group ${prefixCls}-select-group`" >
    <ui-button
      ...
      <ui-icon :type="show?'angle-up' : 'angle-down'" ></ui-icon>
    </ui-button>
      ...
      <ul 
        ...
        v-show="show">
          <li v-if="search">
            ...
            ></input>
            <ui-icon type="plus-square-o" v-if="extra" @click.native="addExtra"></ui-icon>
          </li>
          <li v-if="multiple" :class="`${prefixCls}-select-all`">
            <a @click.prevent="selectAll">
              {{getL('all')}}
             <ui-icon type="check" v-show="allSelected"></ui-icon>
            </a>
          </li>
          ...
          <template v-if="currentOptions.length">
            <li v-for="option in filterOptions" 
              ...
              <a @click.prevent="select(option)" >
                ...
                <ui-icon type="check" v-show="findIndex(option.value) !== -1"></ui-icon>
              </a>
            </li>
          </template>
          <slot v-else ></slot>
...
export default {
  name: 'uiSelect',
  mixins: [valMixin,localeMixin('uiSelect')],
  props: {
    readonly: {
      type: Boolean,
      default: false
