    ...
    <div :class="isFullscreen?'markdown fullscreen':'markdown' " ref="markdown" :style="{height:`${editorHeight}px`}" @mouseover="addListener" @mouseout="removeListener">
        ...
        <ul class="markdown-toolbars">
            ...
            <li v-if="tools.strong" name="粗体">
                <i @click="insertStrong" class="iconfont icon-strong"></i>
            </li>
            <li v-if="tools.italic" name="斜体">
                <i @click="insertItalic" class="iconfont icon-italic"></i>
            </li>
            <li v-if="tools.overline" name="删除线">
                <i @click="insertOverline" class="iconfont icon-overline"></i>
            </li>
            <li v-if="tools.h1" name="标题1">
                <i @click="insertTitle(1)"class="title">h1</i>
            </li>
            <li v-if="tools.h2" name="标题2">
                <i @click="insertTitle(2)"class="title">h2</i>
            </li>
            <li v-if="tools.h3" name="标题3">
                <i @click="insertTitle(3)"class="title">h3</i>
            </li>
            <li v-if="tools.h4" name="标题4">
                <i @click="insertTitle(4)"class="title">h4</i>
            </li>
            <li v-if="tools.h5" name="标题5">
                <i @click="insertTitle(5)"class="title">h5</i>
            </li>
            <li v-if="tools.h6" name="标题6">
                <i @click="insertTitle(6)"class="title">h6</i>
            </li>
            <li v-if="tools.hr" name="分割线">
                <i @click="insertLine" class="iconfont icon-horizontal"></i>
            </li>
            <li v-if="tools.quote" name="引用">
                <i @click="insertQuote" class="iconfont icon-quote"></i>
            </li>
            <li v-if="tools.ul" name="无序列表">
                <i @click="insertUl" class="iconfont icon-ul"></i>
            </li>
            <li v-if="tools.ol" name="有序列表">
                <i @click="insertOl" class="iconfont icon-ol"></i>
            </li>
            <li v-if="tools.code" name="代码块">
                <i @click="insertCode" class="iconfont icon-code"></i>
            </li>
            <li v-if="tools.notChecked" name="未完成列表">
                <i @click="insertNotFinished" class="iconfont icon-checked-false"></i>
            </li>
            <li v-if="tools.checked" name="已完成列表">
                <i @click="insertFinished" class="iconfont icon-checked"></i>
            </li>
            <li v-if="tools.link" name="链接">
                <i @click="insertLink" class="iconfont icon-link"></i>
            </li>
            <li v-if="tools.image" name="图片">
                <i @click="insertImage" class="iconfont icon-img"></i>
            </li>
            <li v-if="tools.table" name="表格">
                <i @click="insertTable" class="iconfont icon-table"></i>
            </li>
            <li v-if="tools.print" name="打印">
                <i class="iconfont icon-dayin" @click="print"></i>
            </li>
            <li v-if="tools.theme" class="shift-theme" name="代码块主题">
                <div>
                    <i class="iconfont icon-yanse" @click="toggleSlideDown"></i>
                    <ul :class="{active:slideDown}" @mouseleave="slideDown=false">
                        <li @click="setThemes('Light')"> <i class="iconfont icon-theme"></i><i>Light</i></li>
                        <li @click="setThemes('Dark')"><i class="iconfont icon-vip"></i><i>VS Code</i></li>
                        <li @click="setThemes('OneDark')"><i class="iconfont icon-atom"></i><i>Atom OneDark</i></li>
                        <li @click="setThemes('GitHub')"><i class="iconfont icon-github51"></i><i>GitHub</i></li>
                    </ul>
                </div>
            </li>
            ...
            <li name="保存到本地" v-show="tools.exportmd">
                <i class="iconfont icon-download" @click="exportMd"></i>
            </li>
            <li v-if="tools.shift&&preview==1" name="预览">
                <i @click="preview=2" class="iconfont icon-preview"></i>
            </li>
            <li v-if="tools.shift&&preview==2" name="编辑">
                <i @click="preview=3" class="iconfont icon-md"></i>
            </li>
            <li v-if="tools.shift&&preview==3" name="分屏显示">
                <i @click="preview=1" class="iconfont icon-group"></i>
            </li>
            <li class="empty"></li>
            <li v-if="tools.fullscreen&&!isFullscreen" name="全屏">
                <i @click="isFullscreen=!isFullscreen" class="iconfont icon-full-screen"></i>
            </li>
            <li v-if="tools.fullscreen&&isFullscreen" name="退出全屏">
                <i @click="isFullscreen=!isFullscreen" class="iconfont icon-exit-full-screen"></i>
            </li>
        </ul>
        <!-- 编辑器 -->
