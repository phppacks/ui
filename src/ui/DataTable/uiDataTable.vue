<template>
  ...
  <div :class="`${prefixCls}-data-table-bar clearfix`">
    ...
    <div v-if="filter && filterArr.length" :class="`pull-left ${prefixCls}-btn-group`">
      ...
      <ui-button
        ...
        type="primary">
        <ui-icon type="reply"></ui-icon>
      </ui-button>
       <ui-button
        :class="`${prefixCls}-data-table-inner-btn`"
        @click.native="goFilter" 
        type="primary">
        <ui-icon type="filter"></ui-icon>
      </ui-button>
    </div>
      <ui-button
        :class="`${prefixCls}-data-table-inner-btn`" 
  ...
  <div>
    <ui-loading center size="lg" v-if="loading"></ui-loading>
    ...
    <div  :class="[loading ? prefixCls + '-data-table-loading':'']" 
      ...
      <table :class="classObj" :style="styleTable">
          ...
          <tbody>
            <tr v-for="(data,index) in list" track-by="uiKey">
...
export default {
  name: 'uiDataTable',
  mixins: [localeMixin('uiDataTable')],
  props: {
    context: {
    },
    selection: {
  ...
  methods: {
    ...
    handlerSource () {
      ...
      for (let i = 0; i < s.length; i++) {
        ...
        ret[i] = Object.assign({}, s[i], {uiKey: i})
        if (this.selection) {
